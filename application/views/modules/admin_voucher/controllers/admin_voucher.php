<?php
class Admin_voucher extends Controller{
    ...
    function index(){
      if ($this->session->userdata('login') == 'admin'){      
           $data['title'] = "Poin";
           $data['noUrut'] = 1;
            $data['data_list'] = $this->m_admin_voucher->get_latest();
		   $data['page'] = "v"; 
	...
	function save(){
		...
		$insert = array(
			...
			'tanggal_mulai' => $thn_start.'-'.$bln_start.'-'.$tgl_start.' '.$hour_start.':'.$menit_start.':00',
			'tanggal_berakhir' => $thn_end.'-'.$bln_end.'-'.$tgl_end.' '.$hour_end.':'.$menit_end.':00',
			'harga' => $harga,
			'persen' => $persen,
			'minimal_transaksi' => $min_trx,
			'maximal_transaksi' => $max_trx
		); 
		
		$this->m_admin_voucher->save($insert);
		
		
        $this->session->set_flashdata('message','Berhasil Diinput!');
        redirect("admin_voucher");	
    }
...
//       if ($this->session->userdata('login_admin') == TRUE){
          $data['title'] = "Edit post";
             ...
             if($id){
                ...
                $data['db'] = $this->m_admin_voucher->getdata($id);
                $data['page'] = "edit";
                $this->load->view("edumix",$data);
             }else {
                 show_404();
...
//       }
	...
	function editsave(){         
		...
		$this->m_admin_voucher->editsave($insert,$id_voucher);
