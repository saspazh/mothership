<?php
class Admin_stat_penjualan extends Controller{
    function  __construct() {
        parent::Controller();
		$this->load->helper(array('form', 'url'));
		$this->load->model('m_admin_stat_penjualan');
    }
    ...
    function index(){
    	if ($this->session->userdata('login') == 'admin'){
           ...
           if($data['t_awal'] AND $data['b_awal'] AND $data['tgl_awal']){
           		$where = "WHERE YEAR(dateflag)=".$data['t_awal']." AND month(dateflag)=".$data['b_awal']." AND day(dateflag)=".$data['tgl_awal'];
           }else{
           		$where = " ";
           ...
           $data['noUrut'] = 1;
           $data['data_list'] = $this->m_admin_stat_penjualan->get_latest();
		   $data['page'] = "v";
    ...
    function indexx(){
      if ($this->session->userdata('login') == 'admin'){
           ...
           $data['noUrut'] = 1;
           $data['data_list'] = $this->m_admin_statistik->get_latest($where);
		   $data['page'] = "v";
           $this->load->view("edumix",$data);
        }else{
             redirect('user/index');
