<?php
class Admin_stok_old extends Controller{
...
//      if ($this->session->userdata('login_admin') == TRUE){
  ...
  //      }else{
	...
	function barcode(){
		...
		$get= $this->m_admin_stok->get_stok();
		foreach($get as $stok){	
			
			$params['data'] = $stok->serial;
			$params['level'] = 'H';
			$params['size'] = 10;
			$params['savename'] = FCPATH.'barcode/'.$stok->serial.'-'.$stok->nama_product.'-'.$stok->color.'-'.$stok->size.'-'.$stok->serialstok.'.png';
			$this->ciqrcode->generate($params);
