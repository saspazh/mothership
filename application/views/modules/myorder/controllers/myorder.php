...
class Myorder extends Controller { // Our Cart class extends the Controller class
	
	function  __construct() {
        parent::Controller();
		$this->load->model('m_myorder');
//		$this->load->library(array('cart')); // Load our cart model for our entire class
	...
	function index(){
		...
		$data['page']='v';
		$data['data_list'] = $this->m_myorder->getorder();
...
//		$query = $this->db->get('barang', 1); // Select the products where a match is found and limit the query by 1
		...
		$this->db->select('a.id_barang, a.id_kategori, c.nama_kategori, b.id_poto, a.nama_barang, a.color, a.price, a.description, b.file_name, b.path, a.waktu');
		$this->db->where('a.id_barang = b.id_barang');
		$this->db->where('a.id_kategori = c.id_kategori');
		$this->db->where('a.id_barang', $id); 
        $query = $this->db->get('barang a, poto b, kategori c',1);
//		return $query->result();
